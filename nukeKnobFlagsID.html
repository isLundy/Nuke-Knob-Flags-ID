...
nuke flags for access from /include/DDImage/Knob.h (NukeX Client >> Help >> Documentation >> C++ Plug-in Development >> Header files).
'''
...
NEVER_DRAW_HANDLES       = 0x0000002000000000 # Disables viewer widget handles from drawing.
                                              # Unlike the NO_HANDLES flag, the state of this flag will never change internally within Nuke
...
NODEGRAPH_ONLY           = 0x0000400000000000 # Indicate that this knob should only be displayed when using the NodeGraph,
                                              # since the Timeline uses gpuEngine, which might not support all the same knobs.
...
EXACT_MATCH_ONLY         = 0x0000000000000002 # Make Enumeration knobs use exact match when setting a value.
                                              # If an attempt is made to set an invalid value, the knob will be put into an Error state.
